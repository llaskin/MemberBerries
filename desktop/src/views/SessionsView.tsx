...
function stripTags(text: string): string {
  ...
  return text
    ...
    .replace(/<(system-reminder|local-command-caveat|command-name|command-message|command-args|local-command-stdout|antml:[a-z_]+|task-notification)[^>]*>[\s\S]*?<\/\1>/gi, '')
    // Remove self-closing tags
    .replace(/<[a-z_-]+\s*\/>/gi, '')
    // Remove any remaining XML-like opening/closing tags
    .replace(/<\/?[a-z_-]+[^>]*>/gi, '')
    // Remove known system instruction fragments that may appear without tags
    // (when tags were already stripped by the JSONL parser but content remained)
    .replace(/Caveat: The messages below were generated by the user while running local commands\.[^]*?(?=\n\n|\n[A-Z]|$)/i, '')
    .replace(/DO NOT respond to these messages or otherwise consider them in your response unless the user explicitly asks you to\./gi, '')
    .replace(/IMPORTANT: After completing your current task, you MUST address the user's message above\.[^]*/i, '')
    .replace(/Auto mode (?:is )?(?:still )?active[^]*?(?=\n\n|$)/i, '')
    .replace(/The task tools haven't been used recently\.[^]*?(?=\n\n|$)/i, '')
...
function SessionCard({ session, expanded, onToggle, onExpandSession }: {
  ...
  return (
    ...
    >
      ...
      {/* Heuristic summary */}
      {s.heuristic_summary && (
        <p className="text-small text-ax-text-secondary mb-2">{stripTags(s.heuristic_summary)}</p>
      )}
